...
impl Discv5Service {
    ...
    pub fn poll(&mut self, cx: &mut task::Context<'_>) -> Poll<(SocketAddr, Packet)> {
        // send messages
        ...
        loop {
            match self.socket.poll_recv_from(cx, &mut self.recv_buffer) {
                Poll::Ready(Ok((length, src))) => {
                    let whoareyou_magic = self.whoareyou_magic;
                    match Packet::decode(&self.recv_buffer[..length], &whoareyou_magic) {
                        Ok(p) => {
