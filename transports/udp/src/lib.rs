//! Implementation of the libp2p `Transport` trait for UDP/IP.
//!
//! # Usage
//!
//! Example:
//!
//! ```
//! extern crate libp2p_udp;
//! use libp2p_udp::UdpConfig;
//!
//! # fn main() {
//! let udp = UdpConfig::new();
//! # }
//! ```
...
/// Represents the configuration for a UDP/IP transport capability for libp2p.
#[derive(Debug, Clone, Default)]
pub struct Udp {}

impl Udp {
    /// Creates a new Udp object for UDP/IP.
    pub fn new() -> Udp {
        Udp {}
    }
}
...
pub struct TcpListener {
    inner: Either<
        Chain<
            Once<ListenerEvent<FutureResult<TcpTransStream, io::Error>>, io::Error>,
            TcpListenStream,
        >,
        Chain<
            IterOk<IntoIter<ListenerEvent<FutureResult<TcpTransStream, io::Error>>>, io::Error>,
            TcpListenStream,
        >,
    >,
}
    ...
    fn poll(&mut self) -> Poll<Option<Self::Item>, io::Error> {
        ...
        loop {
                        ...
                        if let Some(ma) = addrs.get(&addr.ip()) {
                            ...
                            for (ip, ma) in &old_addrs {
                                if !addrs.contains_key(&ip) {
                                    ...
                                    self.pending
                                        .push_back(ListenerEvent::AddressExpired(ma.clone()));
                                }
                            }
                            ...
                            for (ip, ma) in addrs {
                                if !old_addrs.contains_key(&ip) {
                                    ...
                                    self.pending
                                        .push_back(ListenerEvent::NewAddress(ma.clone()));
                                }
                            }
            ...
            match apply_config(&self.config, &sock) {
                ...
                Err(err) => self.pending.push_back(ListenerEvent::Upgrade {
